...
private:
    ...
    void processLine(const std::string &line, int lineNumber)
    {
        ...
        while (std::getline(iss, value, ','))
        {
            ...
            if (position < 11)
            {
                std::pair<int, int> pair = std::make_pair(position, num);
                tupleMap[std::make_tuple(position, num)].push_back(lineNumber); // Mantém std::tuple aqui
                currentLineTuples.push_back(pair);
            }
            else
            {
...
int main()
{
    ...
    DataProcessor dp("/home/aluno/Área de Trabalho/Aeds-Poker-main/Arquivos/poker-hand-training.data");
    dp.processFile();
    std::cout << "-------------------------------------------------" << std::endl;
